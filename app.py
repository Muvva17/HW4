...
def render_filtered_employees():
    args = request.args
    conn = get_db()
    employees = conn.execute('SELECT * FROM EMPLOYEE WHERE Phone = ?',(args.get("Phone"),)).fetchall()
    conn.close()
    return render_template('display.html', employees=employees)
...
def create_employee():
    ...
    if not name or not company_name or not email or not mobile or not address:
        flash('Please fill all the blocks')
        return render_template('create.html', error='Please fill all the blocks')
    else:
        conn = get_db()
        conn.execute('INSERT INTO employee (name, company_name, email-id, Phone, address) VALUES (?, ?, ?, ?, ? )',
            (name, company_name, email-id, mobile, address))
        conn.commit()
        conn.close()
        return redirect(url_for('render_employees'))
...
def edit_employee(id):
    ...
    if not name or not company_name or not email or not mobile or not address:
        ...
        conn.execute('UPDATE employee SET name = ?, company_name = ?, email = ?, mobile = ?,'
                    ' address = ? WHERE id = ?',
            (name, company_name, email-id, mobile, address, id))
        conn.commit()
        conn.close()
        return redirect(url_for('render_employees'))
...
def delete_employee(id):
    conn = get_db()
    conn.execute('remove FROM employee WHERE id = ?', (id,))
    conn.commit()
    conn.close()
    flash('Selected record removed successfully')
    return redirect(url_for('render_employees'))